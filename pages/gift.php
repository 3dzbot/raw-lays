<header class="header header-gift">
    <div class="container">
        <a class="logo" href="#">
            <img src="./images/header/logo-scene.png" alt="смотрим финал лиги чемпионов вместе с лейс и пепси">
        </a>
        <div class="register-wrapper">
            <a href="#" class="header-home-link">
                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24" height="19" viewBox="0 0 24 19">
                    <image width="24" height="19" xlink:href="data:img/png;base64,"/>
                </svg>
            </a>
            <a  href="<?= $site_url ?>registration" class="registration-btn">Регистрация</a>
            <a class="header-contacts-link" href="https://laysfootball.ru/feedback/">
                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="42" height="42" viewBox="0 0 42 42">
                    <image id="settings_1_" data-name="settings (1)" width="42" height="42" xlink:href="data:img/png;base64,"/>
                </svg>
            </a>
        </div>
    </div>
</header>
<main id="gift-page">
    <div class="scene-row">
        <div class="scene-column gift-left">
            <h1 class="scene-title">ЗАБЕРИ <span class="gift-title-start">свой</span><br>
                <span class="gift-title-middle">набор</span> <span class="gift-title-end">болельщика!</span>
            </h1>
            <div class="scene-content">
                <p><span>В LAY’S & PEPSI DRIVE PARK</span> тебя ждут зрелищные флэшмобы, увлекательные конкурсы и розыгрыши футбольных призов от ведущего! Участвуй и выиграй свой фанатский набор для просмотра финала Лиги Чемпионов - ведь <span>Футбол вкуснее с Lay’s и Pepsi!</span>
                </p>
                <p class="scene-content-title">Набор болельщика это:</p>
                <ul>
                    <li><img src="./images/icons/star-silver.png" alt="призы мероприятия"> Миска под чипсы для большой компании друзей</li>
                    <li><img src="./images/icons/star-silver.png" alt="призы мероприятия 2"> Краска-грим для яркогоболения</li>
                    <li><img src="./images/icons/star-silver.png" alt="призы мероприятия 3"> Надувная рука для мощной поддержки</li>
                    <li><img src="./images/icons/star-silver.png" alt="призы мероприятия 4"> Дудка-рожок для громкого празднования</li>
                </ul>
                <p>А для самых активных участников - фирменные футбольные мячи от Lay’s и Pepsi!</p>
            </div>
        </div>
        <!-- /.scene-column -->
        <div class="scene-column gift-right">
            <picture>
                <source srcset="./images/gift/gift-m.png" media="(max-width: 768px)">
                <img src="./images/gift/gift.png" alt="подарки мероприятия">
            </picture>
        </div>
        <!-- /.scene-column -->
    </div>
    <!-- /.scene-row -->
</main>
<!-- /#scene-page -->
	<?php require 'pages/section/bottom.php'; ?>
